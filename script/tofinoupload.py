...
def main():
  commit = subprocess.check_output(['git', 'show', '--format=%h', '-s', '--abbrev=12', 'HEAD']).strip()
...
def upload(path, file):
  ...
  s3put(bucket, access_key, secret_key, os.path.dirname(file), path, [file])
